...
class CatCakePlugin(Star):
    ...
    # ── /找猫糕帮助 ───────────────────────────────────────────────────
    @filter.command("找猫糕帮助")
    async def help_catcake(self, event: AstrMessageEvent):
        yield event.plain_result(
            ...
            "  例：/找猫糕 官服 丹恒\n"
            "  例：/找猫糕 官服 丹恒 5\n"
            "  服务器：官服 B服 亚服 美服 欧服 港澳台 其他 全部\n"
            "  数量上限：9\n\n"
            "/登记猫猫糕 <玩家名> <UID> <猫猫糕1> <猫猫糕2> <猫猫糕3>\n"
            "  登记自己的猫猫糕到友人帐\n"
            "  例：/登记猫猫糕 开拓者 123456789 糯米团 芝麻酥 冰糕\n\n"
            "/登记阿基喵利 <玩家名> <UID>\n"
            "  登记今日限定阿基喵利（凌晨4:00清除）\n"
            "  例：/登记阿基喵利 开拓者 123456789\n\n"
            "/删除猫猫糕 <UID>\n"
            "  删除自己登记的记录\n"
            "  例：/删除猫猫糕 123456789\n\n"
            "查询支持角色名自动映射：\n"
            "  丹恒→糯米团  姬子→星辰拿铁  卡芙卡→墨镜猫咪  银狼→游戏糕手\n"
            "  阿基维利→阿基喵利  开拓者→垃圾糕  阮梅→拉姆之友\n"
            "  等等……输入角色名即可自动查找对应猫猫糕\n\n"
